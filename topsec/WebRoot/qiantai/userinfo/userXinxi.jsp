...
<script language="javascript">
	...
	function closeOpenok() {
		...
		var regName = /^[\u4e00-\u9fa5]{2,4}$/;
		if (!regName.test(document.form1.userRealname.value)) {
			alert("真实姓名填写有误");
			return false;
		}
		...
		if (!mobileRegex.test(document.form1.userTel.value)) {
		...
		if (!myreg.test(document.form1.userEmail.value)) {
			alert("提示:请输入有效的E_mail！");
			return false;
		}

		var reg = /^[1-9]\d{4,9}$/;
		if (!reg.test(document.form1.userQq.value)) {
		...
		<table width="98%" border="0" align="center" cellpadding="0"
			...
			<tr>
				...
				<td bgcolor="#FFFFFF">&nbsp; <input type="text"
					name="userRealname" value="${sessionScope.user.userRealname }" />
				</td>
			</tr>
			<tr>
				<td height="30" align="right" bgcolor="#F9F9F9"
				...
				<td bgcolor="#FFFFFF">&nbsp; <input type="text"
					name="userAddress" value="${sessionScope.user.userAddress }" />
				</td>
			</tr>
			<tr>
				<td height="30" align="right" bgcolor="#F9F9F9"
					...
					<%
				TUser userSex = (TUser) session.getAttribute("user");
				if (userSex.getUserSex().equals("男")) {
			%>
			...
			%>
					<td bgcolor="#FFFFFF">&nbsp; <input type="radio" name="userSex"
					value="男"  />男 &nbsp;&nbsp;&nbsp;&nbsp; <input
					type="radio" name="userSex" value="女" checked="checked"/>女</td>
						<%
				}
			%>
			...
			<tr>
				<td height="30" align="right" bgcolor="#F9F9F9"
					style="font-size: 11px;">E-mail：</td>
				<td bgcolor="#FFFFFF">&nbsp; <input type="text"
			...
			<tr>
				<td height="30" align="right" bgcolor="#F9F9F9"
					style="font-size: 11px;">QQ：</td>
				<td bgcolor="#FFFFFF">&nbsp; <input type="text" name="userQq"
					value="${sessionScope.user.userQq }" />
				</td>
			</tr>
			<tr>
				<td height="30" align="right" bgcolor="#F9F9F9">&nbsp;</td>
				<td bgcolor="#FFFFFF">&nbsp; <input type="hidden" name="userId"
					value="${sessionScope.user.userId }"> <input type="button"
					value="确定" onclick="closeOpenok()" /> <input type="button"
					value="取消" onclick="closeOpen()" />
				</td>
			</tr>
		</table>
	</form>
